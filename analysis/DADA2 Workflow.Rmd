...
table(nchar(getSequences(seqtab_meat))) 
### 409 410 411 412
### 188 82  13288 14830

table(nchar(getSequences(seqtab_dairy)))
### 507 508 509 510    511    512
### 99  136  24 11496  13368  572
...
seqtab_nochim_meat <- removeBimeraDenovo(seqtab_meat, verbose = TRUE)  # 27087 chimeras removed
#################
seqtab_nochim_dairy <- removeBimeraDenovo(seqtab_dairy, verbose = TRUE) # 24073 chimeras removed


# what proportion of counts were removed?
chim_check_meat <- sum(seqtab_nochim_meat)/sum(seqtab_meat) # 0.4611885
frac_removed_meat <- (1-chim_check_meat)*100 # 53.88% of counts were removed from meat group
#################
chim_check_dairy <- sum(seqtab_nochim_dairy)/sum(seqtab_dairy) # 0.6205842
frac_removed_dairy <- (1-chim_check_dairy)*100 # 37.94% of counts were removed from dairy group


### 53.88% of counts were chimeras and removed from meat group.
### 37.94% of counts were chimeras and removed from dairy group.
...
## Meat group
track_meat <- cbind(filtered_out_meat,
               sapply(dada_F_meat, getN),
               sapply(dada_R_meat, getN),
               sapply(merged_amplicons_meat, getN),
               rowSums(seqtab_nochim_meat))

head(track_meat)

# change column names
colnames(track_meat) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "nochim")

head(track_meat)

rownames(track_meat) <- sample_meat

track_meat

##############
## Dairy group
track_dairy <- cbind(filtered_out_dairy,
               sapply(dada_F_dairy, getN),
               sapply(dada_R_dairy, getN),
               sapply(merged_amplicons_dairy, getN),
               rowSums(seqtab_nochim_dairy))

head(track_dairy)

# change column names
colnames(track_dairy) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "nochim")

head(track_dairy)

rownames(track_dairy) <- sample_dairy
...
# Meat group
taxa_meat <- assignTaxonomy(seqtab_nochim_meat, "/workdir/in_class_data/taxonomy/silva_nr99_v138.1_train_set.fa.gz", multithread=FALSE)

# the next line took 3 minutes 
taxa_meat <- addSpecies(taxa_meat, "/workdir/in_class_data/taxonomy/silva_species_assignment_v138.1.fa.gz")
